...
impl Encoding {
    pub(crate) fn decode(&self, bytes: impl AsRef<[u8]>) -> crate::id3::Result<String> {
...
fn string_from_utf16(data: &[u8]) -> crate::id3::Result<String> {
    if data.len() < 2 {
...
fn string_from_utf16le(data: &[u8]) -> crate::id3::Result<String> {
...
fn string_from_utf16be(data: &[u8]) -> crate::id3::Result<String> {
    let mut data2 = Vec::with_capacity(data.len() / 2);
    for chunk in data.chunks_exact(2) {
        let bytes = chunk.try_into().unwrap();
